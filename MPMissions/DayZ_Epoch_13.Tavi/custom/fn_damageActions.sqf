...
if (!isNull _menClose and _hasPatient and !r_drag_sqf and !r_action and !_inVehicle and !r_player_unconscious and _isClose) then {
	...
	_targetBlood = _unit getVariable ["USEC_BloodQty",0];
	...
	if(_unconscious and _hasEpi) then {
		if((_targetBlood < 1000) and (_combattimeout > 0)) then {
		} else {
			r_action = true;
	...
	if (count _crew > 0) then {
		...
		if (_patients > 0) then {
			if (!r_action_unload) then {
				...
				_vehType = typeOf _vehicle;
				_action = _vehicle addAction [format[localize "str_actions_medical_14",_vehType], "custom\medical\load\unLoad_act.sqf",[player,_vehicle], 0, false, true];
				r_player_actions set [count r_player_actions,_action];
			};
		} else {
			if (r_action_unload) then {
