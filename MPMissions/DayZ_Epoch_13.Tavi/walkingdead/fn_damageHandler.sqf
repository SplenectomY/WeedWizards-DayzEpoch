...
scriptName "custom\fn_damageHandler.sqf";
/***********************************************************
...
if (_damage > 0.001) then {
	...
	//if (_unitIsPlayer) then {
		...
		if (DZE_Debug_Damage) then {
			...
			/*if (((r_player_blood - (_damage * _scale)) < 1000) and (r_player_blood > 999)) then {
				r_player_blood = 995;
			};
			if ((r_player_blood < 1000) and (r_player_blood > 0)) then {
				[_unit,(1000 - r_player_blood)] call fnc_usec_damageUnconscious;
			} else { 
				r_player_blood = r_player_blood - (_damage * _scale);
			};*/
		if (r_player_blood - (_damage * _scale) < 1000) then {
			[_unit,5] call fnc_usec_damageUnconscious;
			if (r_player_blood > 999) then {
				r_player_blood = 995;
				diag_log ("PLAYER RECEIVED CRITICAL DAMAGE, BLOOD ADJUSTED TO 995!");
				titleText ["You are in cardiac arrest! You will die without assistance!", "PLAIN DOWN"];
				player groupChat "/scream";
			};
			if (_unconscious) then {
				//r_player_blood = r_player_blood - 3;
				[_unit,(1000 - r_player_blood)] call fnc_usec_damageUnconscious;
				diag_log ("PLAYER WAS INJURED WHILE IN CRITICAL STATE, BLOOD IS " + r_player_blood);
			};
		} else { 
			r_player_blood = r_player_blood - (_damage * _scale);
			diag_log ("PLAYER TOOK DAMAGE NORMALLY, BLOOD IS " + r_player_blood);
		};
	//};
};
...
if ((_damage > 0.4) || (r_player_blood < 1000)) then {	//0.25
	/*
		BLEEDING
	*/
